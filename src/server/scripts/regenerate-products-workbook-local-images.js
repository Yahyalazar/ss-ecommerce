...
const inputWorkbookPath =
  process.argv[2] ||
  "C:/Users/yahya/Downloads/products_50_fixed_attributes_images.xlsx";
...
const batchCode =
  ...
  new Date()
    ...
    .replace(/[-:TZ.]/g, "")
    .slice(2, 12);
const imageFolderName = `import-batch-${batchCode}`;
...
function createProductSvg({
  ...
  const leftCircleX = angle === "Front" ? 118 : 525;
  const leftCircleY = angle === "Front" ? 158 : 128;
  ...
  return [
    ...
    `  <circle cx="${leftCircleX}" cy="${leftCircleY}" r="150" fill="${palette.accent}" opacity="0.18" />`,
...
function main() {
  if (!fs.existsSync(inputWorkbookPath)) {
    throw new Error(`Input workbook was not found: ${inputWorkbookPath}`);
  }
  ...
  const updatedRows = rows.map((row, index) => {
    ...
    const categoryName =
      String(row.categoryName || "").trim() || categorySlug || "Product";
    ...
    const uniqueName = `${originalName} Batch ${batchCode}`;
    const uniqueSku = `${originalSku}-${batchCode}`;
  ...
  const outputSheet = XLSX.utils.json_to_sheet(updatedRows, { header: headers });
  outputSheet["!cols"] = headers.map((header) => {
    if (header === "description") return { wch: 56 };
    if (header === "images") return { wch: 92 };
    if (header === "name") return { wch: 36 };
    if (header === "sku") return { wch: 30 };
    return { wch: 16 };
  });
